...
function matchingBanks(banks, currency) {
  return (banks || []).filter(bank => (bank.currency || 'TWD') === (currency || 'TWD'));
}
...
function selectBankForVoucher(voucher, banks) {
  const available = matchingBanks(banks, voucher.currency || 'TWD');
...
export async function fetchVoucherPaymentSplits(client, voucherId) {
  const { data, error } = await client
    ...
    .select('id, voucher_id, voucher_line_id, payment_recipient_id, bank_account_id, debit_account_id, amount, payment_status, payment_no, payment_sequence_no, recipient_snapshot, currency, exchange_rate, amount_base, paid_at, reversal_date, reversal_reason, revision, recipient:payment_recipients(display_name, identifier, bank_name, bank_branch, account_name, account_number, active), bank:bank_accounts(bank_name, nickname, account_number, currency), payment:voucher_payments(payment_no, payment_sequence_no, status, paid_at, reversal_date, reversal_reason)')
...
function renderBankOptions(banks, currency, selectedId) {
  const available = matchingBanks(banks, currency);
  return [
...
function renderReadOnlySplit(split, recipients, banks, currency) {
  ...
  return `<div class="payment-split-row payment-split-readonly" data-split-id="${split.id}" data-status="${split.payment_status}">
    ...
    <div><strong>${formatMoney(split.amount, currency)}</strong><br><span class="muted">${escapeHtml(bank.nickname || bank.bank_name || '')}</span></div>
  </div>`;
}

function renderEditableSplit(split, recipients, banks, currency) {
  return `<div class="payment-split-row payment-split-editable" data-split-id="${split.id}" data-status="assigned" data-line-id="${escapeHtml(split.voucher_line_id || '')}">
    ...
    <label>付款金額<input type="number" min="0.01" step="0.01" data-split-amount value="${escapeHtml(split.amount)}"></label>
    ...
    <label>公司出款銀行<select data-bank-id>${renderBankOptions(banks, currency, split.bank_account_id)}</select></label>
  ...
  container.innerHTML = `
    <div class="payment-split-editor">
      ...
      <div class="payment-split-totals">
        <div><span>單據總額</span><strong>${formatMoney(voucher.total_amount, currency)}</strong></div>
        <div><span>已付款</span><strong>${formatMoney(paidTotal, currency)}</strong></div>
        <div><span>待付款</span><strong data-assigned-total>${formatMoney(assignedTotal, currency)}</strong></div>
      </div>
        ...
        ${groups.map(group => {
          ...
          return `<section class="payment-split-group" data-line-key="${escapeHtml(group.key)}" data-expected-amount="${escapeHtml(group.expectedAmount)}">
            <div class="payment-split-group-header">
              ...
              <div><strong>${formatMoney(group.expectedAmount, currency)}</strong><span data-line-total>已付 ${formatMoney(paid, currency)}｜待付 ${formatMoney(assigned, currency)}</span></div>
            </div>
            <div class="payment-split-rows">
              ${rows.map(split => split.payment_status === 'assigned'
                ? renderEditableSplit(split, recipients, banks, currency)
                : renderReadOnlySplit(split, recipients, banks, currency)).join('')}
            </div>
      ...
      <div class="payment-split-footer-fields">
        ...
        <label>會計備註<textarea rows="3" data-accounting-note>${escapeHtml(note)}</textarea></label>
      </div>
...
function validateState(state, { requireSelection = false } = {}) {
  const { voucher, recipients, banks, splits } = state;
  const currency = voucher.currency || 'TWD';
  ...
  for (const split of assigned) {
    ...
    if ((bank.currency || 'TWD') !== currency) throw new Error('公司出款銀行幣別必須與單據幣別一致。');
  }
  for (const group of groupDefinitions(voucher)) {
    ...
    if (Math.abs(total - group.expectedAmount) > 0.005) {
      throw new Error(`#${group.index + 1} 的付款拆分合計必須等於 ${formatMoney(group.expectedAmount, currency)}。`);
    }
  }
  ...
  const renderAndWire = () => {
    ...
    container.querySelectorAll('[data-remove-split]').forEach(button => {
      button.addEventListener('click', () => {
        ...
        siblings[0].amount = Math.round((siblings[0].amount + split.amount) * 100) / 100;
        state.splits = state.splits.filter(item => item.id !== split.id);
        renderAndWire();
      });
    });

    container.querySelector('[data-payment-date]')?.addEventListener('change', event => {
      const note = container.querySelector('[data-accounting-note]');
      if (note?.dataset.manual !== '1') note.value = buildDefaultNote(event.target.value, voucher);
    });
