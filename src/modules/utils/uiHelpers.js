...
const WORKFLOW_STATE_BY_STATUS = {
  ...
  partially_paid: ['done', 'done', 'done', 'current'],
...
export function applyRoleBasedTabVisibility() {
  ...
  setTabVisible('arInvoices', ['admin', 'super_admin', 'accounting'].includes(user?.role));
  setTabVisible('arReceipts', ['admin', 'super_admin', 'accounting'].includes(user?.role));
  setTabVisible('arAging', ['admin', 'super_admin', 'accounting'].includes(user?.role));
