...
export async function resetUserPassword(id, password) {
  ...
  if (!sessionData?.session?.access_token) throw new Error('Missing or expired login session. Please sign in again.');
  ...
  if (!res.ok || !result.ok) {
    throw new Error(result.message || `Reset password failed: HTTP ${res.status}`);
  }
  return result;
}
...
export async function inviteNewUser(payload) {
  ...
  const result = await res.json().catch(() => ({}));
  if (!res.ok || !result.ok) {
    throw new Error(result.message || `Invite failed: HTTP ${res.status}`);
  }
  return result;
}
// 預設權限配置（依角色給予預設權限）
