...
- 本機版本 `0.2.41` 尚未推送前，GitHub／Vercel 仍顯示 `0.2.16`。

## 本輪更新

- 會計審核改為每筆報支明細可個別指定會計科目，並提供批次套用到全部明細。
- AI 科目建議改為可針對單筆明細分析，車馬費、住宿費、交通費等會優先建議差旅費，不再一律落到雜項支出。
- 設定頁新增會計科目管理，可新增、修改、刪除 `accounts` 的代碼、名稱與類型。
- Supabase `accounts` 寫入政策補齊 `super_admin`，會計科目管理可由會計、管理員、超級管理員使用。
- 報支與補件重送的付款人欄位改為只輸入身分證/統編，系統會自動帶出中間為 `O` 的遮罩姓名。
- 新增 Supabase RPC `lookup_masked_payee_by_identifier`，只回傳精準查詢到的遮罩姓名，不開放完整付款人主檔給一般員工或主管。
