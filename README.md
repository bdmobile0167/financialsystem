...
- 遠端 AR 核心、外幣匯差、信用額度、角色隔離與既有交易回歸均通過；作廢反轉、帳齡分析、跨幣別收款及正式登入驗收仍待 TASK-022，因此未升 `1.0.0`。
...
- Vercel production 歷史檢查為 `0.2.62` `READY`；目前 `0.2.88` 仍需由使用者推送／部署後重新確認。
- `docs/TASKS_PENDING.md` 已整理為單一待辦清單，完成項目移至 completed。
- AR 客戶、發票與同幣別收款已完成；作廢反轉、帳齡、信用控管與跨幣別收款保留在 `TASK-022`。
