...
- 銀行帳戶主檔的新增、修改、刪除現在與 audit 在同一資料庫交易內完成，audit 不保存完整帳號；財務角色不能直接改寫期初匯率或幣別鎖定欄位。遠端回滾測試已驗證；正式登入與全流程仍待驗收。

## 0.5.4 重點

- 報支單送出前重新確認主管仍屬所選部門；若主管已調整，表單清除失效選項並提示重新選擇。
- 報支表單與銀行卡片延伸美編提案風格，並修正畫面顯示的版本號。
- 本機變更尚未部署；除業務部外，其他部門目前尚無 `manager` 角色主管，需完成角色配置後驗收報支流程。

## 0.5.3 重點

- 員工從報支表單新增付款人與預設收款帳戶，現在會同交易寫入 audit；付款人與收款帳戶不再允許前端直接改資料表。

## 0.5.2 重點

- 撤銷公開資料表的匿名／登入角色 `TRUNCATE` 權限，並封閉交易、銀行流水與總帳的直接寫入入口；資料庫回滾測試確認既有 RPC 流程仍可寫齊資料。

## 0.5.1 重點

- 報支主檔、明細、發票及流程紀錄僅透過資料庫原子 RPC 寫入，直接修改權限已撤銷；資料庫回滾測試通過。

## 0.5.0 重點

- Dashboard 依新版美編提案調整角色列、簽核流程、預算指標與響應式卡片，保留實際資料來源。
- 報支新建與修改重送的主管名單僅顯示同部門主管，切換部門會清除舊選擇。
- 專案費用報表、原子付款與付款人儲存、單據及交易稽核已完成；完整 v1.0.0 驗收進度見 `docs/V1_ACCEPTANCE.md`。
- 本機版本尚未部署；遠端資料未清理，正式登入與角色驗收仍待完成。

## 0.3.0 重點

- 股東權益固定分開顯示公司登記資本、公司資料實收資本、3110 帳載股本及勾稽差額；正式財報只採計資料庫實際分錄。
- 已移除報表層自動補造借銀行／貸股本的舊邏輯，避免公司設定變更後只有報表變動、銀行流水與總帳沒有同步。
- 權益頁可前往修改公司資本資料，或預填本期實際收款的融資／3110 入帳；若金額已含銀行期初餘額，必須走開帳來源勾稽，不可重複新增銀行收入。
- 遠端目前皆為虛擬測試資料：公司資料暫留資本總額 10,000,000、實收資本 900,000，3110 為 0、待勾稽差額 900,000；保留至全流程測試完成後再統一清空，正式值由使用者重新輸入。
- 完整 25 項瀏覽器回歸通過；Production 真實角色驗收與銀行開帳來源分配仍待完成，因此尚未升 `1.0.0`。
...
- 遠端虛擬資本 fixture 仍為資本總額 10,000,000、實收 900,000，本版當時沒有清除或改寫該測試資料。
...
- 此段為 0.2.68 歷史紀錄；目前待部署版本請以 `docs/VERSION.md` 為準。
...
- Vercel production 歷史檢查為 `0.2.62` `READY`；目前 `0.5.5` 尚未部署，需重新確認線上版本。
